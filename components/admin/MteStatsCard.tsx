...
const DimensionBar: React.FC<{ dimension: typeof TLX_DIMENSIONS_INFO[0], score: number }> = ({ dimension, score }) => {
    return (
        <li className="flex items-center space-x-2 text-sm">
            <span className="w-28 text-nasa-gray-300 truncate" title={dimension.title}>{dimension.title}</span>
    ...
    >
      <div className="flex justify-between items-start gap-4">
        <div>
          ...
          <p className="text-xs text-nasa-gray-400 mt-1">Included in: {studyNames.join(', ')}</p>
        </div>
        <div className="text-right flex-shrink-0">
          <p className="text-3xl font-bold text-white leading-none">{numberOfEvals}</p>
          <p className="text-xs text-nasa-gray-400 font-semibold tracking-wider">EVALS</p>
        </div>
      </div>
      ...
      {numberOfEvals > 0 ? (
        <div className="mt-4 pt-4 border-t border-nasa-gray-700 flex-grow grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex flex-col justify-center items-center text-center bg-nasa-gray-800 p-4 rounded-lg">
                ...
                <span className="text-sm font-semibold text-nasa-gray-300 mt-1">Avg. Overall Workload</span>
                <span className="text-xs text-nasa-gray-400">(± {stdDevOverallScore.toFixed(1)} SD)</span>
            </div>
            <div className="md:col-span-2">
                <h4 className="text-sm font-semibold text-nasa-gray-300 mb-2">Workload Drivers (Avg. Raw Scores)</h4>
         ...
         <div className="flex-grow flex items-center justify-center text-center text-nasa-gray-500 py-10">
            <div>
              ...
              <h3 className="mt-2 text-sm font-medium">No evaluation data available</h3>
              <p className="mt-1 text-sm text-nasa-gray-600">This MTE has not been rated yet.</p>
            </div>
        </div>
      )}
    </Card>

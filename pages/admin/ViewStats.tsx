...
const ViewStats: React.FC = () => {
  ...
  }, [ratings, pairwiseComparisons, evaluators, studies]);

  const filteredScores = useMemo(() => {
      if (!filterStudyId) return computedScores;
      return computedScores.filter(score => score.studyId === filterStudyId);
  }, [computedScores, filterStudyId]);
        ...
        <table className="min-w-full divide-y divide-nasa-gray-700">
          ...
          <tbody className="bg-nasa-gray-900 divide-y divide-nasa-gray-700">
            ...
            {filteredScores.length === 0 && (
                <tr>
                    <td colSpan={5} className="text-center py-10 text-nasa-gray-500">No ratings submitted yet.</td>
                </tr>
            )}
          </tbody>
        </table>
